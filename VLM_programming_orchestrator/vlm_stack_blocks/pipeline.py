...
def plan_stacking_trajectory(api_key, base_url, object_to_stack="red block", where_to_stack="the middle of the white paper", config=None):
    ...
    detector = GeminiRoboticsDetector(api_key, base_url=base_url, exposure=exposure, contrast=contrast, brightness_target=brightness_target)
    ...
    if target_position:
        ...
        target_location = detect_target_location(detector, image, where_to_stack)
    
    # Add depth values to detections
    add_depth_to_detections(blocks, depth_image, image_size, hardcoded=True)  # Using hardcoded depth for blocks to avoid issues with missing depth data
    if target_location:
        add_depth_to_detections([target_location], depth_image, image_size, hardcoded=True)  # Using hardcoded depth for target to ensure it's included in the plan even if depth data is missing
    print("\n")
    ...
    trajectory_prompt = build_trajectory_prompt(blocks, target_location, object_to_stack, where_to_stack, stack_order)
    ...
    detector.draw_points_on_image(image, all_waypoints, output_path="2_trajectory_with_waypoints.png")
    
    # Convert to 3D coordinates and save
    stacking_plan_3d = convert_plan_to_3d(detector, stacking_plan_pixel, image_size, depth_frame, depth_intrinsic)
    save_stacking_plan(detector, stacking_plan_3d, "3_stacking_trajectory_plan.yaml")
