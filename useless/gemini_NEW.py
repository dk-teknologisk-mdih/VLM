...
class GeminiRoboticsDetector:
    ...
    def __init__(self, api_key):
        ...
        # self.client = genai.Client(api_key=api_key, http_options={"base_url": "https://yoda.localdom.net:8443/api-proxy/api/v1"})
    ...
    def capture_realsense_image(self, save_path=None):
        ...
        if save_path:
            ...
            # Save depth as text file (raw values in mm)
            ...
            # Save depth as colormap visualization using matplotlib
...
The point is in [y, x] format normalized to 0-1000."""
    ...
    for block in blocks:
        ...
        if depth_value == 0:
            print(f"Warning: Depth value at pixel ({abs_x}, {abs_y}) is 0, which may indicate missing depth data.")
            # Take an average of neighboring pixels to estimate depth - look at 10x10 window
            print(max(0, abs_y-5),min(depth_image.shape[0], abs_y+5))
            print(max(0, abs_x-5),min(depth_image.shape[1], abs_x+5))
            for i in range(max(0, abs_y-5), min(depth_image.shape[0], abs_y+5)):
                for j in range(max(0, abs_x-5), min(depth_image.shape[1], abs_x+5)):
                    print(f"Depth at ({j}, {i}): {depth_image[i, j]} mm")
                    
            neighbors = depth_image[max(0, abs_y-5):min(depth_image.shape[0], abs_y+5), max(0, abs_x-5):min(depth_image.shape[1], abs_x+5)]
            print(neighbors)
            non_zero_neighbors = neighbors[neighbors > 0]
            if len(non_zero_neighbors) > 0:
                depth_value = int(np.mean(non_zero_neighbors))
                print(f"Estimated depth value from neighbors: {depth_value} mm")
            else:
                print("No valid neighboring depth values found, setting depth to -1")
                depth_value = -1  # Indicate unknown depth
...
Remember to consider the height of the stack as you place each block, each block height is 37 mm.
...
def TEST(api_key):
    ...
    image, depth_frame, depth_intrinsic = detector.capture_realsense_image("test_capture.png")
    width, height = image.size
    ...
    # Print depth image as a 2D array for visualization
    np.set_printoptions(threshold=np.inf, linewidth=200)
    print("Depth image (mm):")
    # Save depth image as a 2D array to a txt file
    np.savetxt("depth_image_array.txt", depth_image, fmt='%d')
    print("Depth image array saved as depth_image_array.txt")
    # print(depth_image)  # Commented out terminal print
    # save depth image as a color map for visualization
    import matplotlib.pyplot as plt
    plt.imshow(depth_image, cmap='viridis')
    plt.colorbar(label='Depth (mm)')
    plt.title('Depth Image Visualization')
    plt.savefig("depth_image_visualization.png")
    
    print(f"Depth value at pixel ({abs_x}, {abs_y}): {depth_image[abs_y, abs_x]} mm")
    
    depth_value = depth_image[abs_y, abs_x]  # depth map is in mm
    if depth_value == 0:
        print(f"Warning: Depth value at pixel ({abs_x}, {abs_y}) is 0, which may indicate missing depth data.")
        # Take an average of neighboring pixels to estimate depth - look at 6x6 window
        print(max(0, abs_y-3),min(depth_image.shape[0], abs_y+3))
        print(max(0, abs_x-3),min(depth_image.shape[1], abs_x+3))
        for i in range(max(0, abs_y-3), min(depth_image.shape[0], abs_y+3)):
            for j in range(max(0, abs_x-3), min(depth_image.shape[1], abs_x+3)):
                print(f"Depth at ({j}, {i}): {depth_image[i, j]} mm")
        ...
        if len(non_zero_neighbors) > 0:
            ...
            depth_value = 0  # Indicate unknown depth
    


def main():
    """Example usage of the generic detection functions."""
    # AIzaSyAgR0dH0pXmBKpTGmslMSWI5XdiV2J2GG8
    API_KEY = "AIzaSyAgR0dH0pXmBKpTGmslMSWI5XdiV2J2GG8" #"aip_MDIH_rgK4JXf73YseZB0NJtDG7hKCP30LQAlK"  # Replace with your actual API key
    # # TEST(API_KEY)
    # Example 5: Plan stacking trajectory
    print("=== Planning Stacking Trajectory ===")
    result = plan_stacking_trajectory(
        API_KEY,
        object_to_stack="green and blue blocks",
